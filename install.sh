...
#
# GNOME / KDE のアプリ一覧に「Sensorix」として登録されます。
...
# ------------------------------------------------- legacy name cleanup ------
#: This application used to be called "hwmonitor". Leaving those files behind
#: would show two entries in the application menu after an upgrade.
LEGACY_ID="hwmonitor"

remove_legacy() {
    local removed=0
    local legacy_lib="$PREFIX/lib/$LEGACY_ID"
    for path in "$BIN_DIR/$LEGACY_ID" "$DESKTOP_DIR/$LEGACY_ID.desktop"; do
        if [ -e "$path" ]; then
            rm -f "$path"
            removed=1
        fi
    ...
    for size in 16 22 24 32 48 64 128 256; do
        if [ -e "$ICON_DIR/${size}x${size}/apps/$LEGACY_ID.png" ]; then
            rm -f "$ICON_DIR/${size}x${size}/apps/$LEGACY_ID.png"
            removed=1
        fi
    done
    if [ -e "$ICON_DIR/scalable/apps/$LEGACY_ID.svg" ]; then
        rm -f "$ICON_DIR/scalable/apps/$LEGACY_ID.svg"
        removed=1
    fi
    ...
    if [ "$removed" -eq 1 ]; then
        say "旧名 (hwmonitor) のインストールを削除しました"
    fi
}
...
if [ "$ACTION" = "uninstall" ]; then
    ...
    refresh_caches
    say "完了しました。"
    echo "    設定は残っています。消すには: rm -rf \"\${XDG_CONFIG_HOME:-\$HOME/.config}/sensorix\""
    exit 0
fi

# ----------------------------------------------------------- sanity checks ---
[ -d "$SRC_DIR/sensorix" ] || die "$SRC_DIR に sensorix パッケージが見つかりません"
...
say "アプリ本体をコピーしています..."
mkdir -p "$LIB_DIR"
rm -rf "$LIB_DIR/sensorix"
cp -r "$SRC_DIR/sensorix" "$LIB_DIR/sensorix"
cp "$SRC_DIR/requirements.txt" "$LIB_DIR/requirements.txt"
find "$LIB_DIR/sensorix" -name '__pycache__' -type d -prune -exec rm -rf {} + 2>/dev/null || true
...
# RESOURCE_NAME gives the X11 window the WM_CLASS that sensorix.desktop
# declares in StartupWMClass, so the shell shows the right icon and groups
# the window under its launcher entry.
export RESOURCE_NAME=sensorix
export PYTHONPATH="$LIB_DIR\${PYTHONPATH:+:\$PYTHONPATH}"
exec "$RUNTIME_PYTHON" -m sensorix "\$@"
LAUNCHER_EOF
chmod 755 "$LAUNCHER"
...
say "インストールが完了しました。"
echo
echo "  GNOME : アクティビティ画面で「Sensorix」を検索"
echo "  KDE   : アプリケーションランチャー → システム → Sensorix"
echo "  端末  : $LAUNCHER"
echo
...
case ":$PATH:" in
    ...
    *)
        ...
        echo "    端末から sensorix コマンドを使うには、~/.bashrc などに次を追加してください:"
